...
export default {
  async fetch(req, env, ctx): Promise<Response> {
    ...
    try {
      ...
      // wants to print another copy. Public: it encodes a public profile URL,
      // and an email client can't send an Authorization header anyway.
      const qr = url.pathname.match(/^\/qr\/([0-9a-f-]{36})\.png$/);
      if (qr && (req.method === "GET" || req.method === "HEAD")) {
        const o = await getOrder(env, qr[1]);
        if (!o) return json({ error: "not found" }, 404, cors);
        const png = qrPng(`https://${o.label}.${o.parent}.limo`);
        return new Response(req.method === "HEAD" ? null : png, {
          headers: {
            "Content-Type": "image/png",
            "Content-Length": String(png.length),
            // An order's name never changes, so this image never changes.
...
async function handleCheckout(req: Request, env: Env, user: Session, cors: Cors) {
  ...
  try {
    await env.DB.prepare(
      `INSERT INTO orders (id, user_sub, email, parent, label, contenthash, pet_name, photo_url, amount_cents, status, created_at, updated_at)
       VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, 'pending', ?, ?)`,
    ).bind(id, user.sub, user.email, parent, label, contenthash, petName, photoUrl, amount, now, now).run();
  } catch (e) {
...
async function handleAdmin(req: Request, env: Env, ctx: ExecutionContext, url: URL) {
  ...
  const m = url.pathname.match(/^\/admin\/orders\/([0-9a-f-]{36})\/(retry|revoked|resend-receipt)$/);
  if (req.method === "POST" && m) {
    const o = await getOrder(env, m[1]);
    if (!o) return json({ error: "order not found" }, 404);

    // Re-send the email this order's state already earned — for a customer who
    // lost it, and for checking how a receipt actually renders in a real inbox.
    if (m[2] === "resend-receipt") {
      // ?as=minted|claimed|refunded renders a different one of this order's
      // emails without touching its state — the only way to see how the order
      // confirmation looks once an order has moved past it.
      const as = url.searchParams.get("as");
      const byName = { minted: emailMinted, claimed: emailClaimed, refunded: emailRefunded } as const;
      if (as && !(as in byName)) return json({ error: "as must be minted, claimed or refunded" }, 400);
      const send = as ? byName[as as keyof typeof byName]
        : o.status === "claimed" ? emailClaimed
        : o.status === "refunded" ? emailRefunded
        : ["minted", "claim_requested", "claiming"].includes(o.status) ? emailMinted
        : null;
      if (!send) return json({ error: `no receipt exists for an order that is ${o.status}` }, 409);
      const failure = await send(env, o);
      return json({ ok: !failure, sentTo: o.email, error: failure ?? undefined }, failure ? 502 : 200);
    }
